...
@WebServlet("/ApprovaCandidatoServlet")
public class ApprovaCandidatoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			Integer idCandidato = Integer.parseInt(req.getParameter("idCandidato-approvato"));
			
			AnnuncioCandidato candidatura = bLogic.getCandidatura(idCandidato, idAnnuncio);
			
			if(loggedUser instanceof Azienda) {
				bLogic.setAziendaApprovalOfCandidatura(candidatura, true);
			}
			else if(loggedUser instanceof Gestore) {
				bLogic.setGestoreApprovalOfCandidatura(candidatura, true);
			}
			
			String notifMsg = "Candidato " + candidatura.getCandidato().getNome() + " " + candidatura.getCandidato().getCognome() + " approvato con successo!";
